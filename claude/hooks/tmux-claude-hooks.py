...
# from it ("wprov", not "widget-provisioning-work-wprov") — so you can tell tabs
...
# (anchor-then-task, e.g. "wprov-fix-enrollment"), so the budget is a little
...
# suffix — skipped when picking the one anchor word ("widget-provisioning-work-
# wprov" should anchor on "wprov", never "work"). Only ignored while choosing;
...
def _pick_anchor(words: list) -> str:
    ...
    ("wprov" in "widget-provisioning-work-wprov", "java" in "java-upgrade",
...
def _anchor_word(folder: str) -> str:
    ...
    "widget-provisioning-work-wprov" → "wprov", "irx-dev" → "irx".
...
def _finalize_name(name: str, folder: str) -> str:
    ...
    Two repairs, both enforcing the one-word-location rule:
      - The model echoed the folder ("widget-provisioning-work-planning"):
        the leading run of folder words is collapsed to the single anchor word
        ("wprov-planning") so a multi-word folder never eats the label.
...
def _llm_summarize(prompt: str, folder: str = "") -> str:
    ...
    specific task — e.g. folder "widget-provisioning-work-wprov" + a chat about
    planning → "wprov-planning". A single non-streaming HTTP call to a tiny
    ...
    system = (
        ...
        "('wprov' from 'widget-provisioning-work-wprov', 'java' from "
